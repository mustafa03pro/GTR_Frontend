...
//             // backend controller mapping is /api/purchase/orders
//             const response = await axios.get(`${API_URL}/purchase/orders`, {
//                 headers: { "Authorization": `Bearer ${token}` },
//                 params: { page: 0, size: 200, sort: 'date,desc' } // request a larger page so client-side filtering works
//             });
...
//                     </div>
//                     <Link to="/purchase-dashboard/purchase-orders/new" className="btn-primary flex items-center gap-2 whitespace-nowrap">
...
//                                         <tr key={order.id} className="border-b border-border hover:bg-background-muted">
//                                             <td className="td-cell">{order.date ? new Date(order.date).toLocaleDateString() : ''}</td>
//                                             <td className="td-cell font-medium text-foreground">{order.poNumber || '-'}</td>
...
//                                     ))
//                                 ) : (
//                                     <tr><td colSpan="7" className="text-center py-10 text-foreground-muted"><AlertCircle className="mx-auto h-12 w-12 text-foreground-muted/50" /><h3 className="mt-2 text-sm font-medium text-foreground">No purchase orders found</h3><p className="mt-1 text-sm">Get started by creating a new purchase order.</p></td></tr>
//                                 )}
//                             </tbody>
//                         </table>
//                     </div>
...
//         headers: { Authorization: `Bearer ${token}` },
//         params: { page: 0, size: 200, sort: 'date,desc' }
//       });
//       setPurchaseOrders(res.data.content ?? res.data ?? []);
//     } catch (err) {
//       console.error('Failed to fetch purchase orders:', err);
...
//                     {/* <-- clickable PO number that opens receipt view --> */}
...
//                   </tr>
//                 )) : (
//                   <tr><td colSpan="7" className="text-center py-10 text-foreground-muted"><AlertCircle className="mx-auto h-12 w-12 text-foreground-muted/50" /><h3 className="mt-2 text-sm font-medium">No purchase orders found</h3><p className="mt-1 text-sm">Create a new PO to get started.</p></td></tr>
//                 )}
//               </tbody>
//             </table>
//           </div>
            ...
            <table className="min-w-full">
              ...
              <tbody className="text-foreground-muted">
                {filtered.length > 0 ? filtered.map(order => (
                  ...
                  <tr><td colSpan="8" className="text-center py-10 text-foreground-muted"><AlertCircle className="mx-auto h-12 w-12 text-foreground-muted/50" /><h3 className="mt-2 text-sm font-medium">No purchase orders found</h3><p className="mt-1 text-sm">Create a new PO to get started.</p></td></tr>
                )}
              </tbody>
            </table>
          </div>
